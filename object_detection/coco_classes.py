...
COCO_CLASSES_NAME = {i: n for i, n in enumerate(COCO_CLASSES_LIST)}

# Random RGB colors for each class (useful for drawing bounding boxes)
COCO_COLORS = np.random.uniform(0, 255, size=(len(COCO_CLASSES_LIST), 3)).astype(np.uint8)

_COCO_SELECTION = ['person', 'car', 'bus', 'truck']
COCO_SELECTION = set()

for s in _COCO_SELECTION:
    COCO_SELECTION.add(COCO_CLASS_ID[s])
